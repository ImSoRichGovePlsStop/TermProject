...
public class HealthStationUpgradeUI : MonoBehaviour
{
    public static HealthStationUpgradeUI Instance { get; private set; }
    ...
    public void Refresh()
    {
        var mgr = HealthStationManager.Instance;
        if (mgr == null) return;
    ...
    private void RefreshRows(HealthStationManager mgr)
    {
    ...
    private void RefreshUpgrade(HealthStationManager mgr)
    {
        bool atMax = mgr.CurrentLevel >= mgr.MaxLevel;
        upgradeSection?.SetActive(true);
        upgradeNextText?.gameObject.SetActive(!atMax);
        material1Text?.gameObject.SetActive(!atMax);
        material2Text?.gameObject.SetActive(!atMax);
        ...
        RefreshMaterials(mgr.GetUpgradeCost());
    }

    private void RefreshMaterials(MaterialRequirement[] cost)
    {
        SetMat(material1Text, null, ColGold);
        SetMat(material2Text, null, ColGold);

        if (cost == null || cost.Length == 0)
        {
            SetMat(material1Text, "No materials required", ColGold);
            return;
        }

        var slots = new[] { material1Text, material2Text };
        for (int i = 0; i < Mathf.Min(cost.Length, 2); i++)
        {
            var req = cost[i];
            if (slots[i] == null || req.material == null) continue;
            int have  = GetStock(req.material);
            bool ok   = have >= req.count;
            SetMat(slots[i], $"{req.material.moduleName}  ×{req.count}  (have: {have})", ok ? ColGold : ColRed);
        }
    }

    private static void SetMat(TextMeshProUGUI tmp, string text, Color color)
    {
        if (tmp == null) return;
    ...
    {
        if (HealthStationManager.Instance?.TryUpgrade() == true) Refresh();
    }
}
